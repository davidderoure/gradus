...
On a standard set of 13 just intervals, the simpler asymmetric formula
$f(p/q) = p + \Os(q)$ attains a slightly higher rank correlation with
published human consonance ratings ($\rho = 0.989$ vs.\ $0.979$).
...
harmonic context and partial recognition, which we offer as a conjecture.
\end{abstract}
...
Euler's Gradus Suavitatis \cite{euler1739}, catalogued by the author in OEIS A275314, assigns to each musical
...
\medskip\noindent
\textbf{Organisation.}
Section~2 shows that Gradus admits an exact reinterpretation as a weighted
harmonic coincidence count. Section~3 introduces $\max(p,q)$ and its four
...
Section~7 conjectures a two-stage perceptual interpretation of $f$.
Section~8 surveys the consonance landscape, established scales, and equal
temperament systems, with the caveat that a full treatment would need to
consider the timbre and tuning of instruments. Section~9 develops a
...
Galileo \cite{galilei1638} described consonance in terms of pulse coincidences: two strings
...
vibration period carry equivalent information about perceived consonance, and
that the connection between physics and arithmetic here is deeper than
coincidence.
...
\medskip\noindent
Section~7 develops a conjectured perceptual story behind this decomposition.
...
the conjecture of Section~7 offers one possible motivation.
...
motivated by the conjecture of Section~7. Each formula thus represents a
...
\section{A Two-Stage Perceptual Interpretation}

We conjecture an explanation of the formula by considering a two-stage model
of how the auditory system may evaluate an interval:
...
The mechanism of subharmonic matching --- by which the auditory system infers
a missing fundamental whose upper partials best account for the heard tones
--- is formalised by Parncutt \cite{parncutt1988}, extending Terhardt's
virtual pitch theory to chord roots; Stage~1 of our conjecture draws on
this framework. In that framework the brain does not literally factorise
integers: it performs pattern matching on harmonic templates or, equivalently,
autocorrelation of the acoustic waveform across candidate periods. The prime
decomposition in our formula acts as an \emph{analytical proxy} for this
biological process: $\Os(q)$ counts, in a principled arithmetic way, the
number of octave-reduction and prime-ascent steps needed to reach the implied
fundamental, mirroring the complexity of the template search without claiming
that the auditory system performs explicit prime factorisation.
...
\medskip\noindent
\textbf{Conjectured total perceptual effort:} $f(p/q) = p + \Os(q)$.
...
exactly just. Table~\ref{tab:tet} in Appendix~B lists all 12-TET semitones
alongside their 5-limit just intonation targets and formula values. The
pattern is clear: the fourth and fifth have ET errors of only 2 cents and
low formula values; the thirds and sixths have errors of 14--16 cents and
moderate values; the minor second and tritone have both large errors and the
highest complexity values. Each ET semitone relies on the ear pulling a
...
$f(p/q) = p + \Os(q)$                  & \textbf{0.989} & Asymmetric; two-stage conjecture \\
Optimised asymmetric weights            & 1.000 & Six parameters; likely interpolation on 13 points \\
...
\subsection*{How close is $f$ to the empirical optimum?}
...
\mathrm{A275314}(k) - 1$, recovers Gradus.
...
Both the triangle and this diagonal sequence are proposed as entries in
...
the asymmetric formula $f(p/q) = p + \Os(q)$, $\max(p,q)$, and the
OEIS triangle $T(n,k)$ (Section~10). Each is self-contained and sufficient to
...
\section{12-TET Semitones and Just Intonation Targets}
\label{app:tet}

Table~\ref{tab:tet} shows each 12-TET semitone alongside its standard 5-limit
just intonation target --- the simple ratio that the tempered interval
approximates in diatonic tonal music --- and the dissonance formula values
evaluated at that just target. The error column gives ET cents minus just
cents (positive = ET is sharp of just). Formula values are evaluated at the
just target, not the ET pitch.
...
\toprule
Note & ET (\ct{}) & Just target & Just (\ct{}) & Error (\ct{}) & $G$ & $f$ & $H$ \\
\midrule
...
\noindent
The F\# entry uses $45/32$, the 5-limit diatonic tritone; as discussed in
Section~5, the septimal tritone $7/5$ ($f = 11$) would be a musically
different just target with substantially lower complexity values.
