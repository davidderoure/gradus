...
note. Assigning weight $w(n) = \Os(n)$ to the $n$-th harmonic of each note,
...
weights $w_p(r)$ and $w_q(r)$ for each prime $r \in \{2, 3, 5\}$ in
numerator and denominator respectively --- achieves perfect rank correlation
$\rho = 1.000$. The fitted weights are:
\[
  w_p(2) = 2.64,\quad w_p(3) = 4.44,\quad w_p(5) = 8.62
\]
\[
  w_q(2) = 2.40,\quad w_q(3) = 6.38,\quad w_q(5) = 7.70
\]
The denominator weights for primes 3 and 5 are substantially larger than
their numerator counterparts ($w_q(3)/w_p(3) \approx 1.44$,
$w_q(5)/w_p(5) \approx 0.89$), consistent with the view that the denominator
requires stronger prime-sensitivity than the numerator. The formula $f$, which
sets $w_p(r) = r - 1$ and $w_q(r) = r - 1$ for the denominator while
replacing the numerator sum with the raw integer $p$, is a principled
simplification of this six-parameter family.
...
harmonic series is therefore $p$ itself.
...
$\Delta_{\mathrm{tol}}$ of the just target. The fifth (tolerance $\sim 10$¢
at A3, $\tau = 4$ Hz) is well-approximated by any EDO whose fifth is within
10 cents of 702 cents: this includes 12-TET (2¢ error), 19-TET (7¢), 31-TET
(5¢), and 53-TET (0.07¢), but excludes 5-TET (720¢, error 18¢) and
7-TET (686¢, error 16¢). The major third (tolerance $\sim 6$¢ at A3) is
well-approximated only by 19-TET (1.5¢ error), 31-TET (0.8¢), and 53-TET
(1.4¢); 12-TET (14¢) fails this test at most registers. This arithmetically
...
\]
where the $mp$ factors cancel. The sum diverges formally but is cut off in
practice at the last audible partial $M \approx 10$--$15$, making the total
roughness roughly proportional to $p \cdot f_0 \cdot |\Delta|$ --- the same
scaling as the first partial alone. Focusing on the first coincidence
(at frequency $p \cdot f_0$) is therefore a good approximation; the higher
coincidences add a logarithmic correction that does not change the $1/p$
dependence.
